...
var DefaultConfig = Config{
	...
	HTTPModules:          []string{"net", "web3", "rk", "rotatingking", "randomx", "tvm", "tkminstitution", "tkmaccount"},
	HTTPVirtualHosts:     []string{"localhost"},
	HTTPTimeouts:         rpc.DefaultHTTPTimeouts,
	WSPort:               DefaultWSPort,
	WSModules:            []string{"net", "web3", "rk", "rotatingking", "randomx", "tvm", "tkminstitution", "tkmaccount"},
...
func DefaultDataDir() string {
	...
	if home != "" {
		switch runtime.GOOS {
		case "darwin":
			return filepath.Join(home, "Library", "Tkmchain")
		case "windows":
			// We used to put everything in %HOME%\AppData\Roaming, but this caused
			// problems with non-typical setups. If this fallback location exists and
			// is non-empty, use it, otherwise DTRT and check %LOCALAPPDATA%.
			fallback := filepath.Join(home, "AppData", "Roaming", "Tkmchain")
			appdata := windowsAppData()
			if appdata == "" || common.IsNonEmptyDir(fallback) {
				return fallback
			}
			return filepath.Join(appdata, "Tkmchain")
		default:
