...
var RandomXChainConfig = &ChainConfig{
	...
	// EIP-4844 blob transactions become part of the Antartical gas model.
	// Keeping this on the same timestamp prevents a second activation boundary.
	CancunTime:                   newUint64(MainnetAntarticalTime),
	PragueTime:                   nil,
	OsakaTime:                    nil,
	BPO1Time:                     nil,
	BPO2Time:                     nil,
	...
	UBTTime:                      nil,
	EDATime:                      newUint64(0),
	KyotoTime:                    newUint64(MainnetKyotoTime),
...
var MainnetChainConfig = &ChainConfig{
	...
	GrayGlacierBlock:    nil,
	ShanghaiTime:        newUint64(0),
	// Mainnet blob gas is activated together with Antartical.
	CancunTime:                   newUint64(MainnetAntarticalTime),
	PragueTime:                   nil,
	OsakaTime:                    nil,
	BPO1Time:                     nil,
...
var (
	...
	EgyptChainConfig   = &ChainConfig{
		...
		// Egypt is the Antartical rehearsal network.  Keep the complete
		// privacy and post-quantum rules active from genesis so the testnet
...
// IsEDA returns true if the Emergency Difficulty Adjustment hardfork is active.
func (c *ChainConfig) IsEDA(num *big.Int, time uint64) bool {
...
type Rules struct {
	...
	// require a complete consensus implementation must also consult
	// AntarticalFeatureConsensusReady.
	IsAccountAbstraction, IsParallelExecution, IsAlternativeEVM             bool
	IsFormalVerification, IsMultidimensionalGas, IsBlobGas                  bool
...
func (c *ChainConfig) Rules(num *big.Int, isMerge bool, timestamp uint64) Rules {
	...
	return Rules{
		...
		IsAccountAbstraction:  c.IsAntarticalFeatureActive(FeatureAccountAbstraction, num, timestamp),
		IsParallelExecution:   c.IsAntarticalFeatureActive(FeatureParallelExecution, num, timestamp),
		IsAlternativeEVM:      c.IsAntarticalFeatureActive(FeatureAlternativeEVM, num, timestamp),
		IsFormalVerification:  c.IsAntarticalFeatureActive(FeatureFormalVerification, num, timestamp),
		IsMultidimensionalGas: c.IsAntarticalFeatureActive(FeatureMultidimensionalGas, num, timestamp),
		IsBlobGas:             c.IsAntarticalFeatureActive(FeatureBlobGas, num, timestamp),
		IsNativePrivacy:       c.IsAntarticalFeatureActive(FeatureNativePrivacy, num, timestamp),
		IsStatelessVerkle:     c.IsAntarticalFeatureActive(FeatureStatelessVerkle, num, timestamp),
		IsNativeRandomness:    c.IsAntarticalFeatureActive(FeatureNativeRandomness, num, timestamp),
		IsNativeOracles:       c.IsAntarticalFeatureActive(FeatureOracles, num, timestamp),
		IsCrossChainStandards: c.IsAntarticalFeatureActive(FeatureCrossChain, num, timestamp),
		IsEOF:                 c.IsAntarticalFeatureActive(FeatureEOF, num, timestamp),
