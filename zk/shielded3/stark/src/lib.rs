...
pub fn verify_spend(public: &[u64], words: &[u64]) -> Result<(), String> {
    ...
    let claim = Claim::about_program(&spend_program()).with_input(public);
...
pub fn describe_spend(
    ...
    if path.len() != MERKLE_DEPTH || secret[10..19].iter().any(|&v| v > u32::MAX as u64) {
