...
impl<C: Config> MultiField32ChallengerVariable<C> {
    pub fn new(builder: &mut Builder<C>) -> Self {
        MultiField32ChallengerVariable::<C> {
            ...
            input_buffer: vec![],
            output_buffer: vec![],
            // Bn254 is wider than three 64-bit limbs.  Keep the final
            // partial limb so the recursive transcript sees the full field.
            num_f_elms: C::N::bits().div_ceil(64),
        }
    }
    ...
    pub fn duplexing(&mut self, builder: &mut Builder<C>) {
        ...
        let marker = builder.eval(C::N::from_canonical_usize(self.input_buffer.len() + 1));
        if used < OUTER_MULTI_FIELD_CHALLENGER_RATE {
            self.sponge_state[used] = marker;
            for value in &mut self.sponge_state[(used + 1)..OUTER_MULTI_FIELD_CHALLENGER_RATE] {
                *value = builder.eval(C::N::ZERO);
            }
        } else {
            self.sponge_state[OUTER_MULTI_FIELD_CHALLENGER_RATE] = marker;
        }
        self.input_buffer.clear();
        ...
        self.output_buffer.clear();
        // Only the rate is available as a challenge; capacity remains
        // internal to the sponge.
