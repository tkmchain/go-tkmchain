...
func (pool *LegacyPool) FilterType(kind byte) bool {
	switch kind {
	case types.LegacyTxType, types.AccessListTxType, types.DynamicFeeTxType, types.SetCodeTxType, types.RandomXTxType, types.PQTkmTxType:
		return true
	default:
		return false
	}
...
func (pool *LegacyPool) Pending(filter txpool.PendingFilter) (map[common.Address][]*txpool.LazyTransaction, int) {
	...
	for addr, list := range pool.pending {
		...
		// If the miner requests fee, tip or gas-limit enforcement, cap the lists now.
		if filter.BaseFee != nil || filter.MinTip != nil || filter.GasLimitCap != 0 {
			for i, tx := range txs {
				if filter.BaseFee != nil {
					feeCap := uint256.MustFromBig(tx.GasFeeCap())
					if feeCap.Lt(filter.BaseFee) {
						txs = txs[:i]
						break
					}
				}
