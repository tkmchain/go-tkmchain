...
func (p *BlobPool) Init(gasTip uint64, head *types.Header, reserver txpool.Reserver) error {
	...
	// fully synced).
	state, err := txpool.StateAtWithRetry(p.chain, head)
	if err != nil {
		state, err = txpool.StateAtWithRetry(p.chain, p.chain.Genesis().Header())
	}
	if err != nil {
		return err
	}
...
func (p *BlobPool) Reset(oldHead, newHead *types.Header) {
	...
	resolvedHead, statedb, err := txpool.StateAtWithLatestRetry(p.chain, newHead)
	if err != nil {
