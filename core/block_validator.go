...
func (v *BlockValidator) ValidateBody(block *types.Block) error {
	...
	if (v.config.IsOsaka(block.Number(), block.Time()) || v.config.IsAntartical(block.Number(), block.Time())) && block.Size() > params.MaxBlockSize {
...
func (v *BlockValidator) ValidateState(block *types.Block, statedb *state.StateDB, res *ProcessResult, stateless bool) error {
	...
	if root := statedb.IntermediateRoot(v.config.IsEIP158(header.Number)); header.Root != root {
		if err := statedb.Error(); err != nil {
			return fmt.Errorf("invalid merkle root (remote: %x local: %x) dberr: %w", header.Root, root, err)
		}
