	...
	runDialTest(t, config, []dialTestRound{
		...
		// 9 nodes are discovered, but only 2 are dialed.
		{
			...
			discovered: []*enode.Node{
				newNode(uintID(0x00), "127.0.0.1:3000"), // not dialed because already connected as static peer
				newNode(uintID(0x02), "127.0.0.1:3000"), // ...
				newNode(uintID(0x03), "127.0.0.1:3000"),
				newNode(uintID(0x04), "127.0.0.1:3000"),
				newNode(uintID(0x05), "127.0.0.1:3000"), // not dialed because there are only two slots
				newNode(uintID(0x06), "127.0.0.1:3000"), // ...
				newNode(uintID(0x07), "127.0.0.1:3000"), // ...
				newNode(uintID(0x08), "127.0.0.1:3000"), // ...
			},
		...
		// One dial completes, freeing one dial slot.
		{
			...
			wantNewDials: []*enode.Node{
				newNode(uintID(0x05), "127.0.0.1:3000"),
			},
		},
		...
		// Dial to 0x03 completes, filling the last remaining peer slot.
		{
			...
			discovered: []*enode.Node{
				newNode(uintID(0x09), "127.0.0.1:3000"), // not dialed because there are no free slots
			},
		},
		...
		// (i.e. up to maxActiveDialTasks) are used.
		{
			...
			discovered: []*enode.Node{
				newNode(uintID(0x0a), "127.0.0.1:3000"),
				newNode(uintID(0x0b), "127.0.0.1:3000"),
				newNode(uintID(0x0c), "127.0.0.1:3000"),
				newNode(uintID(0x0d), "127.0.0.1:3000"),
				newNode(uintID(0x0f), "127.0.0.1:3000"),
			},
			wantNewDials: []*enode.Node{
				newNode(uintID(0x06), "127.0.0.1:3000"),
				newNode(uintID(0x07), "127.0.0.1:3000"),
				newNode(uintID(0x08), "127.0.0.1:3000"),
				newNode(uintID(0x09), "127.0.0.1:3000"),
				newNode(uintID(0x0a), "127.0.0.1:3000"),
			},
		},
	})
...
func TestDialSchedNetRestrict(t *testing.T) {
	...
	nodes := []*enode.Node{
		newNode(uintID(0x01), "127.0.0.1:3000"),
		newNode(uintID(0x02), "127.0.0.2:3000"),
		newNode(uintID(0x03), "127.0.0.3:3000"),
		newNode(uintID(0x04), "127.0.0.4:3000"),
		newNode(uintID(0x05), "127.0.2.5:3000"),
		newNode(uintID(0x06), "127.0.2.6:3000"),
		newNode(uintID(0x07), "127.0.2.7:3000"),
		newNode(uintID(0x08), "127.0.2.8:3000"),
	}
	config := dialConfig{
...
func TestDialSchedStaticDial(t *testing.T) {
	...
	runDialTest(t, config, []dialTestRound{
		...
		{
			peersAdded: []*conn{
				{flags: dynDialedConn, node: newNode(uintID(0x01), "127.0.0.1:3000")},
				{flags: dynDialedConn, node: newNode(uintID(0x02), "127.0.0.2:3000")},
			},
			update: func(d *dialScheduler) {
				// These two are not dialed because they're already connected
				// as dynamic peers.
				d.addStatic(newNode(uintID(0x01), "127.0.0.1:3000"))
				d.addStatic(newNode(uintID(0x02), "127.0.0.2:3000"))
				// These nodes will be dialed:
				d.addStatic(newNode(uintID(0x03), "127.0.0.3:3000"))
				d.addStatic(newNode(uintID(0x04), "127.0.0.4:3000"))
				d.addStatic(newNode(uintID(0x05), "127.0.0.5:3000"))
				d.addStatic(newNode(uintID(0x06), "127.0.0.6:3000"))
				d.addStatic(newNode(uintID(0x07), "127.0.0.7:3000"))
				d.addStatic(newNode(uintID(0x08), "127.0.0.8:3000"))
				d.addStatic(newNode(uintID(0x09), "127.0.0.9:3000"))
			},
			wantNewDials: []*enode.Node{
				newNode(uintID(0x03), "127.0.0.3:3000"),
				newNode(uintID(0x04), "127.0.0.4:3000"),
				newNode(uintID(0x05), "127.0.0.5:3000"),
				newNode(uintID(0x06), "127.0.0.6:3000"),
			},
		},
		...
		// two dials are launched to attempt to fill it.
		{
			...
			wantNewDials: []*enode.Node{
				newNode(uintID(0x08), "127.0.0.8:3000"),
				newNode(uintID(0x09), "127.0.0.9:3000"),
			},
		},
		...
		{
			peersAdded: []*conn{
				{flags: inboundConn, node: newNode(uintID(0x07), "127.0.0.7:3000")},
			},
			peersRemoved: []enode.ID{
				uintID(0x01),
			},
...
func TestDialSchedRemoveStatic(t *testing.T) {
	...
	runDialTest(t, config, []dialTestRound{
		...
		{
			update: func(d *dialScheduler) {
				d.removeStatic(newNode(uintID(0x01), "127.0.0.1:3000"))
				d.removeStatic(newNode(uintID(0x02), "127.0.0.2:3000"))
				d.removeStatic(newNode(uintID(0x03), "127.0.0.3:3000"))
			},
			failed: []enode.ID{
				uintID(0x02),
			},
	...
	runDialTest(t, config, []dialTestRound{
		{
			...
			update: func(d *dialScheduler) {
				for id := uint16(0); id < 2000; id++ {
					n := newNode(uintID(id), "127.0.0.1:3000")
					d.addStatic(n)
				}
			},
		},
		{
			...
			wantNewDials: []*enode.Node{
				newNode(uintID(0x0085), "127.0.0.1:3000"),
				newNode(uintID(0x02dc), "127.0.0.1:3000"),
				newNode(uintID(0x0285), "127.0.0.1:3000"),
				newNode(uintID(0x00cb), "127.0.0.1:3000"),
			},
		},
	})
}
...
func TestDialSchedResolve(t *testing.T) {
	...
	resolved := newNode(uintID(0x01), "127.0.0.1:3000")
	resolved2 := newNode(uintID(0x01), "127.0.0.55:3000")
	runDialTest(t, config, []dialTestRound{
		{
			update: func(d *dialScheduler) {
				d.addStatic(node)
...
func TestDialSchedDNSHostname(t *testing.T) {
	...
	node := newNode(uintID(0x01), ":3000").WithHostname("node-hostname")
	resolved := newNode(uintID(0x01), "1.2.3.4:3000").WithHostname("node-hostname")
	runDialTest(t, config, []dialTestRound{
		{
			update: func(d *dialScheduler) {
...
func TestDialSchedPendingInbound(t *testing.T) {
	...
	runDialTest(t, config, []dialTestRound{
		...
		// Discovered nodes 0x03, 0x04, 0x05 but only 0x04 and 0x05 should be dialed.
		{
			...
			discovered: []*enode.Node{
				newNode(uintID(0x03), "127.0.0.3:3000"), // not dialed because pending inbound
				newNode(uintID(0x04), "127.0.0.4:3000"),
				newNode(uintID(0x05), "127.0.0.5:3000"),
			},
			wantNewDials: []*enode.Node{
		...
		// One dial slot remains, node 0x06 is dialed.
		{
			...
			peersAdded: []*conn{
				{flags: inboundConn, node: newNode(uintID(0x03), "127.0.0.3:3000")},
			},
			succeeded: []enode.ID{
				uintID(0x04),
			},
			...
			discovered: []*enode.Node{
				newNode(uintID(0x03), "127.0.0.3:3000"), // not dialed, now connected
				newNode(uintID(0x06), "127.0.0.6:3000"),
			},
			wantNewDials: []*enode.Node{
				newNode(uintID(0x06), "127.0.0.6:3000"),
		...
		// Only 0x03 should be dialed, not 0x07.
		{
			...
			discovered: []*enode.Node{
				newNode(uintID(0x03), "127.0.0.3:3000"),
				newNode(uintID(0x07), "127.0.0.7:3000"), // not dialed because pending inbound
			},
			wantNewDials: []*enode.Node{
				newNode(uintID(0x03), "127.0.0.3:3000"),
			},
