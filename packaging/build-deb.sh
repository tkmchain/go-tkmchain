...
if [[ "$deb_arch" != "amd64" ]]; then
  echo "unsupported Debian architecture: $deb_arch (the portable release currently provides amd64)" >&2
  exit 1
fi
if [[ ! -x build/bin/gtkm ]]; then
  echo "build/bin/gtkm is missing; build gtkm before packaging" >&2
  exit 1
fi
...
install -Dm0755 build/bin/gtkm "$root/usr/bin/gtkm"
ln -s gtkm "$root/usr/bin/tkmchain"
if [[ -x build/bin/shielded-payout-prover ]]; then
  install -Dm0755 build/bin/shielded-payout-prover "$root/usr/bin/shielded-payout-prover"
fi
