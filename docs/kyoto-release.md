...
- Added real voice-call signaling support for number-to-number WebRTC sessions. The node stores and propagates signed encrypted offers, answers, and ICE candidates, while mobile/web clients carry the live audio stream peer-to-peer through WebRTC using the TKM Phone number as the identity.
...
Use the added management APIs from web3 as `bucketPrice`, `operatorKeyPrice`, `mainKingNumberPrice`, `numberSalePrice`, `nextBucketRound`, `bucketGenerationHash`, `generateBuckets`, `buckets`, `listOperators`, `openBucketHash`, `openBucket`, `operatorInventory`, `sellNumber`, `callCandidateHash`, `callCandidateListHash`, `addCallCandidate`, `callCandidates`, `reportOperator`, `addContact`, `contacts`, `blockNumber`, `unblockNumber`, `registerRecovery`, `recoverNumber`, `propagationQueue`, and `importPropagation`.
