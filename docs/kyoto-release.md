...
- Operator keys require a `5000 TKM` payment record and a Main King signed operator grant. When attached to a running chain, the service verifies the payment transaction is canonical, sent by the operator, sent to Main King, and exactly `5000 TKM`. Each accepted operator key automatically receives 50 active phone numbers owned by that operator.
...
- Added operator marketplace listing, operator inventory lookup, `10000 TKM` phone-number sale validation, and signed fraud-report records for operator accountability. A sold number moves from operator ownership to the buyer only after the canonical sale payment transaction is verified as buyer -> operator for exactly `10000 TKM`.
...
Use the added management APIs from web3 as `listOperators`, `operatorInventory`, `sellNumber`, `numberSalePrice`, `reportOperator`, `addContact`, `contacts`, `blockNumber`, `unblockNumber`, `registerRecovery`, `recoverNumber`, `propagationQueue`, and `importPropagation`.
