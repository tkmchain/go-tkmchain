...
Android: install TKM-Wallet-Android-arm64-debug.apk on an ARM64 Android 8+ device. This is a debug-signed testing build. Update an existing installation only when its signing key matches; back up recovery material before any uninstall. Install and start [Orbot](https://orbot.app/) first; Android cannot provide a Tor SOCKS listener by itself. On first start the wallet verifies the bundled `gtkm`, `shielded-payout-prover`, and `libc++_shared.so` assets with SHA-256 and installs them into the app-private `files/bin` directory. It asks Orbot to start, verifies that SOCKS5 can reach a TKM `.onion` bootstrap peer, and only then starts Gtkm with the onion bootnodes and Tor proxy. If `127.0.0.1:9050` is unavailable it does not start a half-connected node; the UI asks you to open Orbot and retry (Orbot's alternate `9150` listener is also accepted). The foreground synchronization service starts the node from private storage, writes diagnostics to `files/node.log`, and keeps the process running while the Activity is minimized. Tap the node status line to view the log; a failed launch is recorded with its exit code instead of leaving the wallet silently disconnected.
