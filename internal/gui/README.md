...
Android: install TKM-Wallet-Android-arm64-debug.apk on an ARM64 Android 8+ device. This is a debug-signed testing build. Update an existing installation only when its signing key matches; back up recovery material before any uninstall. On first start the wallet verifies the bundled `gtkm`, `shielded-payout-prover`, and `libc++_shared.so` assets with SHA-256 and installs them into the app-private `files/bin` directory. The foreground synchronization service starts the node from there, writes diagnostics to `files/node.log`, and keeps the process running while the Activity is minimized. Tap the node status line to view the log; a failed launch is recorded with its exit code instead of leaving the wallet silently disconnected.
