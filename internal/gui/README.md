...
Android: install TKM-Wallet-Android-arm64-debug.apk on an ARM64 Android 8+ device. This is a debug-signed testing build. Update an existing installation only when its signing key matches; back up recovery material before any uninstall. Install and start [Orbot](https://orbot.app/) first; Android cannot provide a Tor SOCKS listener by itself. The release packages `gtkm`, `shielded-payout-prover`, and `libc++_shared.so` as arm64 native payloads so Android extracts them into its executable native-library directory; this avoids `EACCES` failures on devices that mount app files as `noexec`. It asks Orbot to start, verifies that SOCKS5 can reach a TKM `.onion` bootstrap peer, and only then starts Gtkm with the onion bootnodes and Tor proxy. If `127.0.0.1:9050` is unavailable it does not start a half-connected node; the UI asks you to open Orbot and retry (Orbot's alternate `9150` listener is also accepted). The foreground synchronization service keeps the node running while the Activity is minimized and writes diagnostics to `files/node.log`. Use **Download bootstrap** in the top bar to fetch the official archive through Tor and verify its SHA-256 digest. The wallet then asks before stopping the node, moves the existing `chaindata` and `triedb` into a temporary backup, imports the verified archive through block 43,947, and removes the backup only after a successful import. If import fails, the old data is restored. The wallet always asks you to restart before reconnecting, preventing the closed-pipe/offline state caused by replacing an active database. Tap the node status line to view the log; a failed launch is recorded with its exit code instead of leaving the wallet silently disconnected.
