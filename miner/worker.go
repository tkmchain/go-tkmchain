...
func mapTransactionsToLazy(txs map[common.Address]types.Transactions) map[common.Address][]*txpool.LazyTransaction {
	lazy := make(map[common.Address][]*txpool.LazyTransaction, len(txs))
	...
	for {
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				txset := newTransactionsByPriceAndNonce(w.current.signer, mapTransactionsToLazy(txs), w.current.header.BaseFee)
...
func (w *worker) commitTransactions(txs *transactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if w.config.IsLondon(header.Number) {
		header.BaseFee = eip1559.CalcBaseFee(w.config, parent)
	}
	...
	// Only publish an empty sealing task when there are no executable transactions.
	// Otherwise external miners can keep solving the empty task and leave transactions
	// waiting even though they were available for inclusion.
	if len(pending) == 0 {
		if !noempty {
