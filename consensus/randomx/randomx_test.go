...
func TestVerifySealAcceptsCanonicalBlock20179StoredMixDigest(t *testing.T) {
	params.SetActiveCheckpointGenesis(params.MainnetGenesisHash)
	...
	header := canonicalMainnetBlock20179Header()
	if hash := header.Hash(); hash != common.HexToHash("0xba8a7984e885eab29ff3042259233cdfef808aa5869ccb5caf0e24ad2e826f19") {
		t.Fatalf("canonical block 20179 hash mismatch: have %s", hash)
	}
	target := new(big.Int).Div(maxUint256, header.Difficulty)
	if !rx.validStoredMixDigest(header, target) {
		t.Fatal("canonical block 20179 stored mix digest should be below target")
	}
	if err := rx.VerifySeal(verifySealTestChain{config: &config}, header); err != nil {
		t.Fatalf("canonical block 20179 should pass stored-mix compatibility: %v", err)
	}

	mutated := types.CopyHeader(header)
	mutated.Time++
	if rx.allowStoredMixDigestProof(mutated, true) {
		t.Fatal("mutated block 20179 must not use stored-mix compatibility")
	}
}
...
func canonicalMainnetBlock20179Header() *types.Header {
	return &types.Header{
		ParentHash:  common.HexToHash("0x2320fe835d274d461b34eee06fc0d98e8583b1ca3e17e2c9ebfa7bac2c6ec40b"),
		UncleHash:   types.EmptyUncleHash,
		Coinbase:    common.HexToAddress("0x1faa547896a1f89192b1a42a2d5d5bb60b4e0a7d"),
		Root:        common.HexToHash("0xe5e88a008955b74341a44879da1020e249861e2e5bdc0e36f75e88dca5ceef8a"),
		TxHash:      common.HexToHash("0x6e9de9472a47474724c9d494a16c4dc4688660aad6d4d60ad262b5f140e5b967"),
		ReceiptHash: common.HexToHash("0xbbbe73c6307e298ca23c33f1d9f72b414bf1ad338bc49a665b16feb19c659f9d"),
		Difficulty:  big.NewInt(0x51e),
		Number:      big.NewInt(20179),
		GasLimit:    0x8000000,
		Time:        0x6a7add77,
		Extra:       common.FromHex("0xd8830112008467746b6d88676f312e32362e35856c696e7578"),
		MixDigest:   common.HexToHash("0x000a92b8a5c2c0ee6949fe6a26bf07709140c3d7e2ca581fdc9145a22550e171"),
		Nonce:       types.BlockNonce{0xaa, 0xbd, 0xe7, 0xa7, 0x2e, 0xab, 0x49, 0xff},
		BaseFee:     big.NewInt(7),
	}
}
