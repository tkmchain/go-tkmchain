...
const (
	...
	// InitialDifficulty is the per-block difficulty increase during early blocks
...
func (rx *RandomX) CalcDifficultyWithPersistence(chain consensus.ChainHeaderReader, blockTime uint64, parent *types.Header) *big.Int {
	...
	// Linear progression for early blocks (smooth ramp-up). InitialDifficulty is
	// the per-block increase during this phase, keeping difficulty strictly
	// above the genesis value after the first block.
	if currentHeight < LinearProgressionBlocks {
		diff := new(big.Int).Set(GenesisDifficulty)
		increase := new(big.Int).Mul(
			new(big.Int).SetUint64(InitialDifficulty),
			new(big.Int).SetUint64(currentHeight),
		)
		diff.Add(diff, increase)
