...
func DistributeRewards(
	...
	log.Info("========================================")
	log.Info("�� REWARD DISTRIBUTION START")
	log.Info("========================================")
	log.Info("�� Block Information",
		"block", blockNumber,
		"totalReward", FormatANTD(totalReward),
		"totalRewardRaw", totalReward.String())
	...
	log.Info("�� Reward Distribution Breakdown",
		"block", blockNumber,
	...
	if mainKingReward.Sign() > 0 && mainKing != (common.Address{}) {
		...
		log.Info("✅ Main King reward sent",
			"block", blockNumber,
			"address", mainKing.Hex(),
			"amount", FormatANTD(mainKingReward),
			"amountRaw", mainKingReward.String())
	} else {
		log.Warn("⚠️ Main King reward skipped",
			"block", blockNumber,
			"reason", "address empty or reward zero")
	}
	...
	if rotatingKingReward.Sign() > 0 && rotatingKing != (common.Address{}) {
		...
		log.Info("✅ Rotating King reward sent",
			"block", blockNumber,
			"address", rotatingKing.Hex(),
			"amount", FormatANTD(rotatingKingReward),
			"amountRaw", rotatingKingReward.String())
	} else {
		log.Warn("⚠️ Rotating King reward skipped",
			"block", blockNumber,
			"reason", "address empty or reward zero")
	}
	...
	if minerReward.Sign() > 0 && miner != (common.Address{}) {
		...
		log.Info("✅ Miner reward sent",
			"block", blockNumber,
			"address", miner.Hex(),
			"amount", FormatANTD(minerReward),
			"amountRaw", minerReward.String())
	} else {
		log.Warn("⚠️ Miner reward skipped",
			"block", blockNumber,
			"reason", "address empty or reward zero")
	}
...
func DistributeUncleReward(stateDB *state.StateDB, uncleCoinbase common.Address, uncleNumber, blockNumber uint64, blockReward *big.Int) {
	...
	if adjustment.Sign() > 0 {
		...
		log.Info("Uncle reward distributed",
			"uncleNumber", uncleNumber,
			"blockNumber", blockNumber,
			"uncleAddress", uncleCoinbase.Hex(),
			"reward", FormatANTD(uncleReward),
			"rewardRaw", uncleReward.String())
	}
}

// ==================== Functions (Will place some where else later ====================
