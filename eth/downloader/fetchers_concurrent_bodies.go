...
const downloaderRequeueTTL = 2 * time.Minute
...
func (q *bodyQueue) reserve(peer *peerConnection, items int) (*fetchRequest, bool, error) {
...
func (q *bodyQueue) deliver(peer *peerConnection, packet *eth.Response) (int, error) {
        ...
        // Convert RLP lists to slices
        txs := make([][]*types.Transaction, len(*resp))
        uncles := make([][]*types.Header, len(*resp))
        for i, body := range *resp {
                // body.Transactions is rlp.RawList[*types.Transaction]; convert to slice
                txs[i] = body.Transactions.Items()
                uncles[i] = body.Uncles.Items()
        }
