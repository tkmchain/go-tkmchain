...
// SubscribeSyncEvents subscribes to synchronization lifecycle events.
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	defer func() {
		...
		if err != nil {
			...
			d.syncFeed.Send(SyncEvent{Type: SyncCompleted, Mode: d.mode})
		}
	}()
	if p.version < 62 {
		return errTooOld
...
func (d *Downloader) fetchHeight(p *peerConnection) (*types.Header, error) {
    ...
    // Request the header by hash
    req, err := p.peer.RequestHeadersByHash(head, 1, 0, false, resCh)
    if err != nil {
        return nil, err
    }
    defer req.Close()
    ...
    case res := <-resCh:
        ...
        // Extract headers - the response is *eth.BlockHeadersRequest, not []*types.Header
        headers, ok := res.Res.(*eth.BlockHeadersRequest)
        if !ok {
            p.log.Debug("Failed to extract headers from response", "type", fmt.Sprintf("%T", res.Res))
            return nil, errBadPeer
        }
        
        // Convert BlockHeadersRequest to []*types.Header
        ...
        header := (*headers)[0]
        p.log.Debug("Successfully retrieved remote chain height", "number", header.Number, "hash", header.Hash())
        return header, nil
...
func (d *Downloader) findAncestor(p *peerConnection, remoteHeader *types.Header) (uint64, error) {
	...
	go p.peer.RequestHeadersByNumber(uint64(from), count, skip, false, nil)
	...
	for start+1 < end {
		...
		go p.peer.RequestHeadersByNumber(check, 1, 0, false, nil)
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64, pivot uint64) error {
	...
	getHeaders := func(from uint64) {
		...
		if skeleton {
			...
			go p.peer.RequestHeadersByNumber(from+uint64(MaxHeaderFetch)-1, MaxSkeletonSize, MaxHeaderFetch-1, false, nil)
		} else {
			p.log.Trace("Fetching full headers", "count", MaxHeaderFetch, "from", from)
			go p.peer.RequestHeadersByNumber(from, MaxHeaderFetch, 0, false, nil)
		}
	}
