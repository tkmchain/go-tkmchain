...
func (q *receiptQueue) reserve(peer *peerConnection, items int) (*fetchRequest, bool, error) {
...
func (q *receiptQueue) unreserve(peer string) int {
	fails := q.queue.ExpireReceipts(downloaderRequeueTTL)
	if fails[peer] > 2 {
...
func (q *receiptQueue) deliver(peer *peerConnection, packet *eth.Response) (int, error) {
	blobs := *packet.Res.(*eth.ReceiptsRLPResponse)
	...
	for i, blob := range blobs {
		...
		receipts[i] = make([]*types.Receipt, len(stored))
		for j, receipt := range stored {
			receipts[i][j] = (*types.Receipt)(receipt)
		}
	}
